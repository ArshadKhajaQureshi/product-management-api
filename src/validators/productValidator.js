...
const handleValidationErrors = (req, res, next) => {
  ...
  next(new ApiError(400, result.array().map((err) => err.msg).join("; ")));
};
...
const createFieldRules = [
  ...
  body("sku").custom(isPresent).withMessage("sku is required").bail().custom(isString).withMessage("sku must be a string"),
...
const updateFieldRules = [
  ...
  body("sku").optional().custom(isString).withMessage("sku must be a string"),
